			...
			public void actionPerformed(ActionEvent e) {
				...
				if(num.equals("")) {
					...
					if(c==1) {
						...
						JOptionPane.showMessageDialog(null, "编号不正确或者已经超三分钟", "消息",JOptionPane.WARNING_MESSAGE);
					}
				...
				if(stockNum.getText().equals("")) {
					...
					if(cmbSupName.getSelectedIndex()==0) {
						...
						if(c==1) {
							...
							System.out.println(c);
							JOptionPane.showMessageDialog(null, "请检查编号输入格式或者已经超过3分钟", "消息",JOptionPane.WARNING_MESSAGE);
						}
